...
type PageName = "dashboard" | "assets" | "api";
...
document.addEventListener("DOMContentLoaded", (): void => {
  ...
  const hash: string = window.location.hash.substring(1) || "dashboard";
  showPage(hash as PageName);
});

function setupNavigation(): void {
  const navItems = document.querySelectorAll<HTMLElement>(".nav-item");

  navItems.forEach((item: HTMLElement): void => {
    item.addEventListener("click", (e: Event): void => {
      ...
      // Update active state
      navItems.forEach((nav: HTMLElement) => nav.classList.remove("active"));
...
function showPage(pageName: PageName): void {
  ...
  pages.forEach((page: HTMLElement) => page.classList.remove("active"));
...
function updateDashboardStats(assets: Asset[]): void {
  const totalAssets: number = assets.length;
  const portfolioValue: number = assets.reduce(
    (sum: number, asset: Asset): number => {
...
function displayDashboardAssets(assets: Asset[]): void {
...
function displayEmptyDashboard(): void {
  const container = document.getElementById("dashboardAssets");
  if (!container) return;
...
function updateApiStatus(status: string, type: ApiStatus): void {
...
async function loadAssetsTable(): Promise<void> {
...
function displayAssetsTable(assets: Asset[]): void {
  const tbody = document.getElementById("assetsTableBody");
  if (!tbody) return;
...
async function testEndpoint(method: string, endpoint: string): Promise<void> {
...
function refreshData(): void {
  ...
  const pageId = currentPage.id as PageName;
...
function editAsset(id: number): void {
...
(window as any).openAddAssetModal = openAddAssetModal;
